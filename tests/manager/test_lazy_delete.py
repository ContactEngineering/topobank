"""
Tests for soft-delete bookkeeping on Surface and Topography.
...
@pytest.mark.django_db
def test_topography_lazy_delete_records_deleting_user():
    user = UserFactory()
    surface = SurfaceFactory(created_by=user)
    topo = Topography1DFactory(surface=surface)
...
def test_default_manager_still_hides_soft_deleted_objects():
    ...
    assert Topography.all_objects.filter(pk=topo.pk).exists()
