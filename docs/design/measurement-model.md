...
The plan above was implemented in one change (see the `measurements` package,
the `Measurement` model and migrations 0082-0085). Where the implementation
deviates from the plan, or discovered something the plan did not anticipate:

- **Steps 1 and 2 of §9 were combined.** The intermediate phase - measurement
  types reading the *old* columns through an adapter - was skipped: it would
  have meant writing an adapter layer whose only purpose was to be deleted in
  the same release. Under the lockstep decision there is no deployment between
  the two steps, so the adapter bought nothing. The migration sequence within
  the release is unchanged: rename, add fields, backfill, drop columns.
- **A `channel_index_hint` column was added.** §5 says a legacy container's
  integer `data_source` is "resolved to a channel name on first inspection",
  which requires carrying the index that far. A transient attribute would not
  survive the hand-off to the Celery worker, so the index is persisted in a
  dedicated nullable column that the first inspection consumes and clears. It
  exists only for containers written before channels were named. The
  alternative - falling back to the reader's default channel - would silently
  select different data for any file whose default is not the recorded index,
  which is the exact failure mode this change removes.
- **The backfill normalizes values the old columns never constrained.** `unit`,
  `detrend_mode`, `fill_undefined_data_mode` and `instrument_type` were
  `TextField`s with `choices`, which Django enforces in forms but not in the
  database, and `full_clean` was never called on save. The new schemas *do*
  constrain them, so rows holding e.g. `"um"` instead of `"µm"`, or the empty
  instrument type the old container importer wrote, would have become
  unparseable. The backfill maps known spellings, clears unrecognizable units
  (leaving the measurement with incomplete metadata, which the next inspection
  repairs from the file) and prints what it corrected. Null-valued instrument
  parameters are dropped for the same reason: `InstrumentParametersModel`
  accepts a missing key but rejects an explicit null.
- **`exclude_none` is required when serializing, not optional.** Same root
  cause as above: metadata dumped with nulls cannot be read back. All writes go
  through one `dump_metadata` helper.
- **The container key was renamed to `measurements`.** Import reads both it and
  the legacy `topographies`. §8 called for a format bump but did not say
  whether the key moves; renaming it keeps the archive consistent with the
  model, and third-party consumers can detect the layout from `versions`.
- **`squeezed_datafile` kept its name.** §6 floated generalizing it to
  `cache_datafile`. It is now a *capability* of the measurement type
  (`has_canonical_file`), which is what mattered; renaming the column would
  have added migration churn for a cosmetic gain.
- **The built-in types are also registered from the `manager` app.** A
  deployment that forgets to add `topobank.measurements` to `INSTALLED_APPS`
  would otherwise have no registered kinds at all, which fails in a confusing
  way. Entry-point discovery still requires the app.
- **The three built-in factories were split by kind, which corrected a
  misnaming.** `Topography1DFactory` used `line_scan_1.asc`, which is a
  *nonuniform* line scan - so the old "1D" factory had been producing
  nonuniform data all along. There are now `NonuniformLineScanFactory` (that
  file), `UniformLineScanFactory` (a genuinely uniform 1D file) and
  `TopographyMapFactory`, giving each kind its own coverage.

Not implemented, still open:

- **The attribute guard from §5** (comparing a resolved channel's `dim` and
  units against those recorded at the last inspection, and refusing on a
  mismatch). It was marked "recommended, can be added after the initial
  release" and needs no schema change, since `file_info["channels"]` already
  records what it would compare.
- **Generic `scalar-field-*` kinds** for non-height channels (§10). The
  machinery is in place - such channels are listed with `kind: null` and become
  importable the moment a type claims them - but no type ships yet.
- **Publishing datasets containing an unknown-kind measurement** (§10) is
  unchanged pending the policy decision; nothing in the implementation blocks
  it.
