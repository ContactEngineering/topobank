...
def test_plugins_available_for_user():
    organization = OrganizationFactory(name="My University", plugins_available="topobank_statistics, topobank_contact")
    user = UserFactory()
...
def test_signal_for_default_group_association():
    from ..models import DEFAULT_GROUP_NAME, DEFAULT_ORGANIZATION_NAME
