...
def import_measurement(topo_meta: TopographyMeta, topo_file, surface):
    ...
    # The archive's metadata goes into the `metadata` document. `kind` is left out
    # on purpose: nothing has inspected the file yet, so which kind this is only
    # becomes known on the first read. `size_y` is omitted rather than set to None
    # for the same reason -- a line scan's schema has no such field, and an
    # explicit None would be rejected once the kind is known.
    metadata = {
        "size_x": size_x,
        "unit": topo_meta.unit,
