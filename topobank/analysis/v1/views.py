...
#     *
#     * A trained model instance is the combination of a workflow template(settings) and an analysis that has been named(named result).
#     '''
...
#     def get_queryset(self):
#         """
#         Get the queryset for the trained models.
#         """
#         # TODO: Update to org filter?
#         return TrainedModel.objects.all()

#     def map_function_to_prediction(self, function: str) -> str:
#         mapping = {
#             'sds_ml.gaussian_process_regression':
#                 'sds_ml.prediction.gaussian_process_regression',
#             'sds_ml.gaussian_process_classification':
#                 'sds_ml.prediction.gaussian_process_classification',
#             'sds_ml.gaussian_process_regression.v2':
#                 'sds_ml.prediction.gaussian_process_classification.v2',
#             'sds_ml.gaussian_process_classification.v2':
#                 'sds_ml.prediction.gaussian_process_classification.map.v2',
#         }
#         return mapping.get(function, "") # empty string is falsy

#     @extend_schema(request=None) #fixes swagger requiring body for this endpoint.
#     @action(methods=["POST"], detail=False, url_path="new/(?P<analysis_id>[^/.]+)")
#     def custom_create(self, request, analysis_id: int):
#         '''
#         * Creates a new trained model.
#         *   POST /api/trained-model/new/{analysis_id}/
#         *       Creates a new trained model for the given analysis ID.
#         *       The analysis must be a named analysis (have a name set).
#         *       If the analysis does not exist or is not named, a 400 error is returned.
#         *       Creates a new WorkflowTemplate and attaches it to the trained model.
#         *
#         * @input:
#         *   request: The request object.
#         *   analysis_id: The ID of the analysis to create a trained model for.
#         *
#         * @output:
#         *   Responses: 
#         *             (TrainedModel, 201 Created)
#         *             (Errors, 400 Bad Request)
#         *
#         '''
#         analysis = Analysis.objects.get(id=analysis_id)
#         if not analysis or not analysis.name:
#             return Response({"error": "Analysis not found or is not named."}, status=400)
        
#         # TODO: check if Analysis.result works here instead (models.py 258)
#         folder = Folder.objects.get(id=analysis.folder.id)
#         unique_categories = folder.read_json("result.json")['unique_categories']


#         workflow_template = WorkflowTemplate.objects.create(
#             name=f"Configuration for {analysis.name}",
#             unique_categories=unique_categories,
#             implementation=analysis.function,
...
#         }
#         trained_model = TrainedModel.objects.create(**model_data)
#         trained_model.permissions = analysis.permissions
#         trained_model.save()

#         prediction_name = self.map_function_to_prediction(analysis.function.name)
#         if prediction_name:
#             prediction_function = AnalysisFunction.objects.get(name=prediction_name)
#             workflow_template.kwargs = prediction_function.get_default_kwargs()
#             # workflow_template.prediction_implementation = prediction_function
#         workflow_template.trained_model_id = trained_model.id
#         workflow_template.save()

#         serializer = self.get_serializer(trained_model)
#         if serializer.data:
#             return Response(serializer.data, status=201)
#         else:
#             return Response(serializer.errors, status=400)
    
#     #TODO: Add ?name parameter so user can specify a name for the copy.
#     @extend_schema(request=None) #fixes swagger requiring body for this endpoint.
#     @action(methods=["POST"], detail=False, url_path="copy/(?P<trained_model_id>[^/.]+)")
#     def copy(self, request, trained_model_id: int):
#         '''
#         * Creates a copy of an existing trained model.
#         *   POST /api/trained-model/copy/{trained_model_id}/
#         *       Creates a new trained model that is a copy of the existing one.
#         *       The new trained model will have a new ID and will be independent of the original.
#         *
#         * @input:
#         *   request: The request object.
#         *   trained_model_id: The ID of the trained model to copy.
#         *
#         * @output:
#         *   Responses:
#         *       (TrainedModel, 201 Created)
...
#         *
#         '''
#         original_trained_model = TrainedModel.objects.get(id=trained_model_id)
#         if not original_trained_model:
#             return Response({"error": "Trained model not found."}, status=400)
        
#         copied_workflow_template = WorkflowTemplate.objects.create(
#             name=f"Copy of {original_trained_model.workflow_template.name}",
#             unique_categories=original_trained_model.workflow_template.unique_categories,
#             implementation=original_trained_model.workflow_template.implementation,
#             kwargs=original_trained_model.workflow_template.kwargs,
#             creator=request.user
#         )

#         # Create a copy of the original trained model
#         copied_trained_model = TrainedModel.objects.create(
#             name=f"Copy of {original_trained_model.name}",
#             analysis=original_trained_model.analysis,
#             creator=request.user,
#             workflow_template=copied_workflow_template
#         )
#         copied_trained_model.permissions = original_trained_model.permissions
#         copied_trained_model.save()

#         copied_workflow_template.trained_model_id = copied_trained_model.id
#         copied_workflow_template.save()

#         serializer = self.get_serializer(copied_trained_model)
#         if serializer.data:
#             return Response(serializer.data, status=201)
#         else:
#             return Response(serializer.errors, status=400)

#     @extend_schema(exclude=True) #Removes endpoint from swagger
#     def create(self, request, *args, **kwargs):
#         return Response(status=405) #Method not allowed. Use custom_create instead.
