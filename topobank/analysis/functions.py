...
def power_spectrum(topography, window=None, tip_radius=None, progress_recorder=None, storage_prefix=None):
    ...
    try:
        ...
        if (len(exc.args) > 0) \
           and (exc.args[0] == 'Positions not sorted.' or ("is reentrant" in exc.args[0])):
            raise ReentrantTopographyException("Cannot compute PSD for reentrant surfaces.")
        # Maybe it would be better if SurfaceTopography threw an exception with more explicit class
        raise
