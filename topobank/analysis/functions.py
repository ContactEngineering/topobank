...
def average_series_list(series_list, num_points=100, xscale='linear', yscale='linear'):
    ...
    xscale, yscale: str
                Scaling of axis on which the linear interpolation is done. Can be 'linear' or 'log'.
    ...
    #
    # If xscale or yscale is not 'linear', the interpolation
    # needs to be done on another scale. Afterwards the results
    # have to be transformed back to the original scale
    #
    if xscale == 'log':
        xscale_func = np.log
        reverse_xscale_func = np.exp
    elif xscale == 'linear':
        xscale_func = lambda arr: arr
        reverse_xscale_func = xscale_func
    else:
        raise ValueError(f"Averaging for xscale '{xscale}' not yet  implemented.")
    if yscale == 'log':
        yscale_func = np.log
        reverse_yscale_func = np.exp
    elif yscale == 'linear':
        yscale_func = lambda arr: arr
        reverse_yscale_func = yscale_func
    else:
        raise ValueError(f"Averaging for yscale '{yscale}' not yet  implemented.")

    # Now we know minimum and maximum x and can calculate an x vector for interpolation
    scaled_av_x = np.linspace(xscale_func(min_x), xscale_func(max_x), num=num_points)

    # do linear interpolation for each series and average y values of all relevant series
    scaled_interpol_y_list = []
    for s in series_list:
        interpol = interp1d(xscale_func(s['x']), yscale_func(s['y']),
                            bounds_error=False)  # inserts NaN outside of boundaries
        scaled_interp_y = interpol(scaled_av_x)
        scaled_interpol_y_list.append(scaled_interp_y)

    scaled_av_y = np.nanmean(scaled_interpol_y_list, axis=0)  # ignores NaNs generated by interpolation
    scaled_std_err_y = scipy.stats.sem(scaled_interpol_y_list, axis=0, nan_policy='omit')
    return {
        'name': common_name,
        'x': reverse_xscale_func(scaled_av_x),
        'y': reverse_yscale_func(scaled_av_y),
        'std_err_y': reverse_yscale_func(scaled_std_err_y)
    }
...
def power_spectrum_for_surface(surface, window=None, tip_radius=None, num_points=100,
    ...
    for series_list in topo_result_series.values():
        result['series'].append(average_series_list(series_list, num_points=num_points,
