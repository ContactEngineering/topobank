...
def average_series_list(series_list, num_points=100, xscale='linear'):
    ...
    std_err_y = scipy.stats.sem(interpol_y_list, nan_policy='omit').filled(0)
    # a masked value in result of sem() only appears if there is for a given index only one
    # number (others are nan). So the error here should be zero, because there is only
    # one value for the mean, which is effectively the mean.
    return {
        'name': common_name,
        'x': av_x,
        'y': av_y,
