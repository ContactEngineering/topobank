...
def average_series_list(series_list, num_points=100):
    ...
    av_x = np.linspace(min_x, max_x, num=num_points)
    ...
    std_err_y = scipy.stats.sem(interpol_y_list, axis=0, nan_policy='omit')
    return {
        'name': common_name,
        'x': av_x,
        'y': av_y,
...
@register_implementation(name="Power Spectrum", card_view_flavor='plot')
def power_spectrum_for_surface(surface, window=None, tip_radius=None, progress_recorder=None, storage_prefix=None):
    ...
    max_x = None  # maximum x value for all series when measured in unit 'xunit'
    min_x = None  # .. same for minimum x
    ...
    for topo_idx, topo in enumerate(topographies.all()):
        ...
        for s in topo_result['series']:
            ...
            topo_result_series[series_name].append(s)
    ...
    for series_name, series_list in topo_result_series:
        result['series'].append(average_series_list(series_list))

        # if xunit is None:
        #     xscale = 1
        #     xunit = topo_result['xunit']
        # else:
        #     xscale = ureg.convert(1, topo_result['xunit'], xunit)
        #
        # for s in topo_result['series']:
        #     if len(s['x']) > 0:
        #         max_series_x = max(s['x'])
        #         min_series_x = min(s['x'])
        #         if max_x:
        #             max_x = max(max_x, xscale * max_series_x)
        #             min_x = min(min_x, xscale * min_series_x)
        #         else:
        #             max_x = xscale * max_series_x
        #             min_x = xscale * min_series_x
        #
        # topo_results.append(topo_result)
        # progress_recorder.set_progress(topo_idx + 1, num_topographies)
