...
/**
 * Submit an ajax call for updating a card in the analysis view.
 *
 * @param card_url {String} URL to call in order to get card content as HTTP response
 * @param card_wrapper_element_id {String} CSS id of the div element containing the card (card wrapper)
 ...
 */
function submit_analyses_card_ajax(card_url,
      ...
      $.ajax({
        ...
        success : function(data, textStatus, xhr) {
          ...
          if ((0 === call_count) || (200 === xhr.status) ) {
            $(jquery_card_wrapper_selector).html(data); // insert resulting HTML code as inner HTML of the selected element
          }
          if (202 === xhr.status) {
        ...
        error: function(xhr, textStatus, errorThrown) {
          // console.error("Error receiving response for card '"+card_wrapper_element_id+"'. Status: "+xhr.status
          ...
          if ("abort" !== errorThrown) {
              ...
              let details = {
                  ...
                  card_url: card_url,
                  card_wrapper_element_id: card_wrapper_element_id,
              ...
              `;
              $(jquery_card_wrapper_selector).html(message);
              $(jquery_card_wrapper_selector).addClass("alert alert-error");
          }
        }
      });
}
